...
## Sync
- **Upstream**: merge upstream `decolua/9router` up to `v0.5.86` into this fork,
  integrating every incoming feature (Xiaomi MiMo desktop login, Claude Opus 5.5,
  System One decision endpoint, OpenCode Zen, Qoder CN, OpenRouter/Vertex video,
  expanded CLI tools, combo presets, i18n) while preserving this fork's own work
  (CI npm publish as `@dipandhali2021/9router`, qwen/fal-ai/replicate media
  providers, proxy-pool relay import, provider Playground + Capabilities cards,
  structured live console log page, per-request ids, optional PostgreSQL backend,
  AES-256-GCM encryption at rest). Fork and CLI versions aligned at `1.5.86`
