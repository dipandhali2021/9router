...
export async function saveRequestUsage(entry) {
  try {
    ...
    // transaction. The SQLite adapters serialize statements, and the Postgres
    // adapter runs this SERIALIZABLE with retry, so the read-modify-write of
    // usageDaily and the lifetime counter cannot lose a concurrent update.
    // `inserted` is returned rather than assigned outward because a Postgres
    // retry re-runs this callback.
