 ...
 */
export async function handleStreamingResponse({ providerResponse, provider, model, providerLabel, sourceFormat, targetFormat, userAgent, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, reqLogger, toolNameMap, customToolNames, streamController, onStreamComplete, streamDetailId, pxpipe, reqTag, reqId, log, credentials }) {
  if (onRequestSuccess) {
    Promise.resolve()
  ...
  if (upstreamContentType && !upstreamContentType.includes('text/event-stream') && !upstreamContentType.includes('application/json')) {
    ...
    if (log?.errorLine) log.errorLine(reqTag, "✗", `BLOCKED ${status} · ${providerLabel || provider}/${model} · non-SSE (${upstreamContentType})\n    ${shortMsg}`, reqId);
 ...
 */
export function buildOnStreamComplete({ provider, model, connectionId, apiKey, requestStartTime, body, stream, finalBody, translatedBody, clientRawRequest, pxpipe, reqTag, reqId, log }) {
  ...
  const onStreamComplete = (contentObj, usage, ttftAt) => {
    ...
    if (log?.line) log.line(reqTag, "📊", formatDoneLine({ usage, latency }), reqId);
  };
